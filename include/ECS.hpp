...
class Scene
{
    ...
    private:
        ...
        void*    _getMut(uint64_t id, const std::type_info& ti,
                     std::size_t size, std::size_t align) const;
        void     _addSet(uint64_t id, const std::type_info& ti,
                     const void* data, std::size_t size, std::size_t align);
        void     _remove(uint64_t id, const std::type_info& ti) const;

        void     _setParent(uint64_t id, uint64_t parentId); 
        void     _forEachChildOpaque(uint64_t parentId,
...
template<class T>
inline Entity& Entity::Add(const T& value) {
...
inline const T* Entity::TryGet() const {
    return _scene->_has(_id, typeid(T))
        ? static_cast<const T*>( const_cast<Scene*>(_scene)
                ->_getMut(_id, typeid(T), sizeof(T), alignof(T)) )
        : nullptr;
}
template<class T>
inline void Entity::Remove() {
    _scene->_remove(_id, typeid(T));
}

inline Entity& Entity::SetParent(Entity parent) {
    _scene->_setParent(_id, parent.RawId());
    return *this;
}
